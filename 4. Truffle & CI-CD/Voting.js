...
contract('Voting', accounts => {
    ...
    describe('tallyVotes', () => {
        beforeEach(async function () {
            ...
            VotingInstance = await Voting.new({from:owner});
            await VotingInstance.addVoter(owner, {from:owner});
            await VotingInstance.addVoter(second, {from:second});
            await VotingInstance.addVoter(third, {from:third});
            await VotingInstance.startProposalsRegistering({from:owner});
            await VotingInstance.addProposal('Jojo est le meilleur manga', {from:owner});
        ...
        it.skip("...verify winningProposal", async () => {
            await VotingInstance.setVote(0, {from:owner});
        ...
        it.skip("...should not be able to tally", async () => {
    ...
    describe('change workflow', () => {
        ...
        it('switch to RegisteringVoters', async () => {
            await VotingInstance.startProposalsRegistering({from:owner});
        })       
    })
})
